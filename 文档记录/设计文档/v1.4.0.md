...
| 文件 | 变更 | 说明 |
|------|------|------|
| 设置页新增"媒体修复"开关 | 新增 | 调用 `GET/PUT /api/v1/config`（admin） |
| `src/services/file_service.py` | 修改 | `stream_range` 支持 `range_start_seconds`（声明 time 支持） |
| `src/components/stream_proxy.py` | 修改 | time 修复流适配：meta 兼容（file_size/duration）、GET 200+chunked、字节 Range 线性映射时间（方案 B） |
...
|------|------|------|
| 设置页新增"媒体修复"开关 | 新增 | 调用 `GET/PUT /api/v1/config`（admin） |
| `lib/utils/stream_proxy.dart` | 修改 | time 修复流适配：与桌面端同构（含 `mapRangeToSeconds` 映射） |
| `lib/services/file_service.dart` | 修改 | `streamRange` 支持 `rangeStartSeconds` |
...
- **time 模式 meta 帧额外字段（方案 B）**：`file_size`（原文件大小）、`duration`（媒体时长秒），供桌面/移动 StreamProxy 构造 Content-Length 与线性时间 seek 映射；`codec`（fMP4 真实 codec 串）供 Web MSE 组装 SourceBuffer MIME。
- **声明机制（方案 B）**：仅请求携带 `range_start_seconds` 字段（即使为 0）时才可能返回 time 修复流；未携带的旧客户端一律 byte 原文件流，零回归。
