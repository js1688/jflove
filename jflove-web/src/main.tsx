...
import { registerStreamProxy, syncStreamSession } from './utils/stream-proxy';
import './index.css';
...
// 注册 Service Worker 流式代理（视频/音频边下边播；仅安全上下文可用，失败不影响主流程）
registerStreamProxy();

// 免登录恢复后预建立加密会话（session_key 不持久化）。
// 复用 http-client 的单飞 resyncSession，与请求时的惰性建立共用同一锁，
// 避免并发触发两次密钥交换导致会话冲突。会话建立后同步给 SW 流式代理。
if (useAuthStore.getState().isLoggedIn) {
  resyncSession()
    .then(() => syncStreamSession())
    .catch(() => {
