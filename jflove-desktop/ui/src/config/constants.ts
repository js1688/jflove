 ...
 * 默认服务端地址。
 *
 * **v1.5.0 反馈修复：默认改为空串 = 同源**（请求走相对路径，如 `/api/v1/...`）。
 *
 * 为什么：原先写死 `http://localhost:8989`，而 Web 端通常部署在另一个源
 * （不同端口或域名）⇒ 每个请求都变成**跨源请求**，浏览器会先发一条
 * `OPTIONS` 预检，服务端实际收到的 HTTP 条数是业务调用的 **2 倍**。
 * 实测（`设计预览/diag_web_dup_requests.mjs`，32 个场景）：`OPTIONS` 数恒等于
 * 业务请求数 —— 这就是用户反馈「感觉像调接口调两遍」的真实来源之一。
 *
 * 同源部署时由 nginx 反代 `/api/` 到后端（见 `nginx.conf`）；本地开发由
 * `vite.config.ts` 的 dev proxy 承担。用户仍可在登录页/设置页填写绝对地址
 * （那就回到跨源，safari/Firefox 会走预检，属用户显式选择）。
 */
