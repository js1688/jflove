---
name: web-frontend
description: 负责 jflove-web 浏览器端应用开发（页面、组件、路由、交互逻辑）。当前模块标记为"本次先不开发"，需用户明确要求方可启用。
---
...
启用 web 端时**必须实现**与桌面端等价的应用层加密：

- 浏览器端用 `WebCrypto API` 实现 X25519 + ChaCha20-Poly1305 + HKDF-SHA256（与桌面端、服务端三端兼容）；不允许引入第三方加密库的"等效替代"算法。
- 必须有等价的 `http_client.ts` 封装：自动加密请求 body、解密响应 body、识别加密错误信封、按帧解密文件流。所有 API 调用统一走它，组件层禁止直接 `fetch()`。
- 文件下载/预览必须按服务端的帧加密协议解析 `[4B 长度][12B nonce][密文+16B tag]`。WebCrypto `crypto.subtle.decrypt` 配合 `ReadableStream` 边收边解密。
- 浏览器是高敏环境（XSS、扩展程序、开发者工具都能读到内存），session_key 必须存在不可序列化结构（JS Closure / IndexedDB 加密存储），**禁止**`localStorage.setItem("session_key", ...)`。
- 浏览器端首次接触本项目时，需要在设计文档中评估是否引入"服务端公钥固定"——XSS 风险高于桌面端，可能需要重新与用户讨论 §9.2 第 7-8 条。
